...
export default class GoogleProvider extends LocationProvider {
  ...
  async search(query, lat, lng) {
    const res = await apiClient.get(
      `/places/search?input=${encodeURIComponent(query)}${lat ? `&lat=${lat}` : ''}${
        lng ? `&lng=${lng}` : ''
      }`
    );
  ...
  async reverseGeocode(lat, lng) {
    const res = await apiClient.get(`/places/reverse-geocode?lat=${lat}&lng=${lng}`);
    return {
  ...
  async getDetails(placeId) {
    const res = await apiClient.get(`/places/details?placeId=${placeId}`);
    return {
      address: res.data?.address,
      lat: res.data?.lat,
      lng: res.data?.lng,
